...
public class CalTask3 extends RecursiveTask<Map<String, Integer>>{
    ...
    public CalTask3(byte[] buff, int start, int end) {
        this.buff = buff;
        this.start = start;
        this.end = end;
    }
    ...
    protected Map<String, Integer> compute() {
        if(end - start < LIMITLEN) {
            ...
            CalTask3 left = new CalTask3(buff, start, mid);
            left.fork();
            CalTask3 right = new CalTask3(buff, mid, end);
    ...
    public Map<String, Integer> parseBuffer(int start, int end) {
        ...
        if(indexEnd == fileLen)
            indexEnd = fileLen - 1;
        int dataLen = (int)(indexEnd - indexStart + 1);
